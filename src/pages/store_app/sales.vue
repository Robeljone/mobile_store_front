...
const fetchData = async () => {
  try {
    ...
    const res = await api.get('/store/getSales')
    purchase_da.value = res?.data?.purchase?.map((da: any) => ({
      id: da.id,
      Suplier: da.cust_name.name,
...
const save_purchase = async () => {
  const respo = await api.post('/store/addSales', form.value).then((res) => {
    if (res.status === 200) {
        ...
        <div class="flex flex-col md3">
          <div class="item">
            <VaSelect v-model="form.suplier" class="mt-4 border-2 border-sold border-grey-900 px-2" type="text"
              label="Customer" :options="suplier_list" value-by="id"
