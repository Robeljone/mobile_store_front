...
const fetchData = async () => {
  try {
    ...
    const res = await api.get('/store/getSuplier')
...
const save_suplier = async () => {
  const respo = await api.post('/store/addSuplier', form.value).then((res) => {
    if (res.status === 200) {
...
const update_suplier = async () => {
  const respo = await api.post('/store/updateSuplier', uform.value).then((res) => {
    if (res.status === 200) {
