...
const columns = defineVaDataTableColumns([
  ...
  { label: 'Defined Price', key: 'Defined Price', sortable: true },
  { label: 'Purchase Price', key: 'Purchase Price', sortable: true },
  { label: 'Created_at', key: 'created_at', sortable: true },
  { label: 'Action', key: 'actions' },
])

const { init } = useToast()
const model_da = ref<Model[]>([])
...
const fetchData = async () => {
  try {
    ...
    const res = await api.get('/store/getModel')
    console.log(res)
    model_da.value = res?.data?.data?.map((da: any) => ({
      id: da.id,
      Name: da.name,
      Brand: da.brand.name,
      Status: '',
...
const save_model = async () => {
  const respo = await api.post('/store/addModel', form.value).then((res) => {
    if (res.status === 200) {
...
<template>
  ...
  <VaCard>
    <VaCardContent>
      ...
      <DataTable :columns="columns" :items="model_da" :loading="tableLoad" :on-edit="editItem" :on-delete="deleteItem"
  ...
  <VaModal v-model="doShowAddModal" size="large" mobile-fullscreen close-button hide-default-actions>
    ...
    <br></br>
    <VaForm ref="myForm" submit.prevent="save_model">
      <div class="row">
      ...
      <div class="row">
        <div class="flex flex-col md3">
          <div class="item">
            <VaSelect v-model="form.brand" class="mt-4 border-2 border-sold border-grey-900 px-2" type="text"
              label="Brand" :options="brand_list" value-by="id"
              text-by="name" clearable />
          </div>
        </div>
      </div>
      <br></br>
      <VaButton size="small" class="mr-6 mb-2" @click="save_model">
        Save
      </VaButton>
    </VaForm>
  </VaModal>
